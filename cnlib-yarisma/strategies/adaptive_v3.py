"""
Adaptive V3 — Ultra-volatile rejim eklendi.

V2 altcoin'de (std > 5%) çökerdi. V3'te 4. rejim:
  ULTRA_VOLATILE: std_daily > 4.5% → sadece baseline 1x + SMA50 filter.
  Çünkü yüksek vol + düşük autocorr = hem signal hem leverage kontrolü gerekir.

Rejim hiyerarşisi:
  ULTRA_VOL: std > 4.5% → safe mode (1x leverage, SMA50 gate, flat bias)
  SWAP:      autocorr > 0.20 → V6 MaxConc swap
  HYBRID:    autocorr > 0.10 → orta risk
  ROBUST:    default (SMA50 gate + 2x)

Tuned parameters (grid search winner):
  mm=0.008, mc=0.95, cb=3.0, at=0.20
"""
...
class AdaptiveV3(BaseStrategy):
    WARMUP = 30
    AUTOCORR_WINDOW = 60
    REGIME_REFRESH = 30
    AUTOCORR_SWAP_THRESHOLD = 0.20
    AUTOCORR_HYBRID_THRESHOLD = 0.10
    ULTRA_VOL_THRESHOLD = 0.045        # std daily > 4.5% → safe mode

    # Swap modu (V6 MaxConc tuned)
    SWAP_MIN_MAG = 0.008
    ...
    # Ultra-safe (ultra-volatile rejim)
    SAFE_SMA_LONG = 50
    SAFE_MAX_LEV = 1
    SAFE_EXTREME = 0.10
    ...
    def _measure_autocorr_and_vol(self, data):
        corrs = []
        vols = []
        for coin in COINS:
            df = data[coin]
            ...
            if pd.notna(ac):
                corrs.append(ac)
            vols.append(r.std())
        autocorr = sum(corrs) / len(corrs) if corrs else 0.0
        vol = sum(vols) / len(vols) if vols else 0.0
        return autocorr, vol

    def _refresh_regime(self, data):
        ac, vol = self._measure_autocorr_and_vol(data)
    ...
    def predict(self, data: dict[str, Any]) -> list[dict]:
        ...
        if self.current_regime == "safe":
            return self._predict_safe(data)
        elif self.current_regime == "swap":
    ...
    def _predict_safe(self, data):
        """Ultra-volatile: 1x leverage, SMA50 strict, extreme 10% flat."""
        signals = {}
        for coin in COINS:
            df = data[coin]
            ...
            # Son 5 candle'da single-day > 10% varsa flat
        ...
        for coin in COINS:
            if signals[coin] == 1:
                decisions.append({
                    "coin": coin, "signal": 1,
                    "allocation": per_alloc, "leverage": self.SAFE_MAX_LEV,
                })
            else:
